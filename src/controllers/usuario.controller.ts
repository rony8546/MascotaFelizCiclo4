...
export class UsuarioController {
  ...
  async identificarUsuario(@requestBody() credenciales: Credenciales) {
    let u = await this.servicioAutenticacion.indetificarUsuario(
    ...
    if (u) {
      ...
      throw new HttpErrors[401]('Los datos de autentificacion son invalidos');
    }
  }
  ...
  async create(
    ...
    //Norificacion al usuario
    let destino = usuario.correo;
    let asunto = 'Registro de la plataforma';
    let contenido = `Hola ${usuario.nombre}, su nombre de usuario es: ${usuario.correo}, y su clave es ${clave}`;
